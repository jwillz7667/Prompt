import type { CapabilityItem, Modality } from './types.js'

export type ModalityDetail = {
  name: string
  description: string
  placeholder: string
  example: string
...
export const modalityDetails: Record<Modality, ModalityDetail> = {
  text: {
    name: 'Text',
    description: 'Refine prompts for writing, research, analysis, chat, and general assistant workflows.',
    placeholder:
      "Paste the text task you want improved. Example: 'Write a launch email for a new AI feature aimed at product managers.'",
  ...
  image: {
    ...
    placeholder:
      "Paste the scene you want improved. Example: 'A luxury perfume bottle on black marble with dramatic rim lighting.'",
  ...
  video: {
    ...
    placeholder:
      "Paste the video concept you want improved. Example: 'A futuristic sports car drifting through neon Tokyo for a short ad.'",
  ...
  audio: {
    ...
    placeholder:
      "Paste the audio concept you want improved. Example: 'An uplifting synth-pop anthem with glossy 80s production.'",
  ...
  code: {
    ...
    placeholder:
      "Paste the coding task you want improved. Example: 'Build a secure Express auth API with JWT refresh tokens and tests.'",
  ...
  '3d': {
    ...
    placeholder:
      "Paste the 3D concept you want improved. Example: 'A stylized medieval sword game asset with worn steel and leather grip.'",
...
export function getModalityDetail(modality: Modality): ModalityDetail {
  return modalityDetails[modality]
}

export function enrichModalityCapabilities(modalities: CapabilityItem[]): CapabilityItem[] {
  return modalities.map((item) => {
    const detail = modalityDetails[item.id as Modality]
